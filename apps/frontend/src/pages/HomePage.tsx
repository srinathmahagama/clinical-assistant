...
import { Heart, Stethoscope, Users, Shield, ArrowRight, UserPlus, User } from 'lucide-react';
...
const HomePage: React.FC<HomePageProps> = ({ onNavigate, onLogout, onContinueAsGuest, user, isGuest }) => {
    ...
    <p className="text-lg leading-relaxed text-white/90 whitespace-pre-line">
      {t('aboutText')}
            ...
            {/* <div className="grid md:grid-cols-3 gap-8 mb-20">
              {features.map((feature, index) => (
                <div key={index} className="backdrop-blur-sm rounded-xl p-8 border bg-white/15 border-white/30">
      ...
      {showWelcomeModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`rounded-2xl max-w-md w-full p-8 relative ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'
            }`}>
            {/* Close Button */}
            <button
              onClick={handleCloseModal}
              className={`absolute top-4 right-4 transition-colors ${theme === 'dark'
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={handleContinueAsGuest}
                className={`w-full py-3 px-6 rounded-xl font-semibold transition-all ${theme === 'dark'
